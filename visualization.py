...
with st.container():
    ...
    if show_dataBtn:
        ...
        st.dataframe(data=sample, width=800)
...
st.write("Make Bar Chart and Pie Using Sex Counts")
...
with st.expander("CHECK SEX TABLE COUNTS"):
...
st.markdown("---")
st.subheader("Sex Distribution 'Static Viz'")
...
with st.container():
    ...
    st.header("Select Feature 'Dynamic Viz'")
...
st.markdown("---")
st.subheader("Visualization The Distribution Total Bill, Group by \
    Sex and Day")
...
st.markdown("---")
st.subheader("Interactive Visualization Using Multi Widget")
